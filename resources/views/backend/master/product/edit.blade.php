        ...
        <div class="col-12">
            @component('components.backend.card.card-form')
                ...
                @slot('content')
                    ...
                    <x-forms.select name="categories_id" id="categories_id" label="{{ __('button.select') }} {{ __('menu.category') }}"
                        :isRequired="true">
                        @foreach ($data['category'] as $category)
                            <option value="{{ $category->id }}"
                                {{ $data['product']->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}
                            </option>
                        @endforeach
                    </x-forms.select>
                    ...
                    <x-forms.input type="number" name="stok" id="stok" :label="__('field.stoke')" :isRequired="true" stok
